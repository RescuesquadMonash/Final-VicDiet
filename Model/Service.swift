    ...
    func fetchFoodNutrient(completion: @escaping (_ food: FoodObject, _ number: Int) -> ()){
        for i in 0...25{
            foodRef.child("\(i)").observe(.childAdded, with: { (snapshot) -> Void in
                ...
                let surveyID = dictionary?["Survey ID"] as? NSNumber
                
                let food = FoodObject(name: Name, protein: Protein!, surveyFlag: Survey, calcium: Calcium!, totalFat: fat!, sodium: Sodium!, vitaminC: C!, vitaminA: A!,surveyID: surveyID!)
                completion(food,i)
            })
        }
    }
